...
bool UFGVisionSensingComponent::IsPointVisible(const FVector& PointToTest, const FVector& Origin, const FVector& Direction, float DistanceMinimum) const
{
	...
	if(bIsValid)
	{
		...
		GetWorld()->LineTraceSingleByChannel(hit, GetOwner()->GetActorLocation(), PointToTest, TraceChannel,
			QueryParams, FCollisionResponseParams::DefaultResponseParam);

		if(hit.bBlockingHit && FVector::Distance(hit.Actor->GetActorLocation(), PointToTest) < 10)
		{
			return true;
		}
